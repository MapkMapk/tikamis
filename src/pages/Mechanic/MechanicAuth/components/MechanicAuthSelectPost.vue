  ...
  <section :class="{'!h-auto':isPostsVisible}"
    ...
    <div class="relative">
      <div :class="{'border-green': isPostsVisible}"
        ...
        <div @click="isPostsVisible = !isPostsVisible"
             class="flex w-full items-center justify-between h-[80px] pl-5 pr-5 cursor-pointer">
          <span :class="{'!text-black': isPostSelected}" class="text-gray-a1a4ad text-2xl">{{isPostSelected ? selectedPost : "Выберите пост"}}</span>
          <SvgIcon :class="{'rotate-180':isPostsVisible}" name="arrow-down" class="w-[25px] h-[15px]" />
        </div>
      </div>
    ...
    <BaseButton class="w-[337px] bg-green text-white mt-[30px]" @click="formHandler">Войти
    </BaseButton>
...
function formHandler() {
  isPostSelected.value ? emit('returnPostNumber', selectedPost.value) : errorText.value = 'Вы не выбрали пост'
}
