<template>
  <section
    v-if="modalBooleanStore.isActive"
    class="z-50 flex justify-center items-center absolute w-full h-screen left-0 top-0 bg-gray-848484"
  >
    <div class="flex flex-col items-center bg-white w-[520px] h-[288px]">
      <div class="flex flex-col items-center mt-[70px] mb-[60px]">
        <span class="font-medium text-3xl">{{ props.mainTitle }}</span>
        <span class="text-lg mt-6">{{ props.mainText }}</span>
      </div>
      <div class="flex w-full mt-auto">
        <component :is="primaryButtonComponent">go</component>
        <BaseButtonFilledLight></BaseButtonFilledLight>
      </div>
    </div>
  </section>
</template>
<script setup>
import BaseButtonFilledLight from '@/components/BaseButtonFilledLight.vue'

let props = defineProps({
  mainTitle: String,
  mainText: String,
  primaryButtonComponent: Object,
  primaryButtonText: String,
  SecondaryButtonText: String
})
</script>
